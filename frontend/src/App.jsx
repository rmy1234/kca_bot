...
function App() {
  ...
  async function loadSavedQuestions(scope, status, limit) {
    ...
    try {
      ...
      else if (result.length < limit) setNotice('조건에 맞는 저장된 문제가 ' + result.length + '문항뿐이라 모두 불러왔습니다.')
...
function generationNotice({questions, requested, rejected, unverified}) {
  ...
  if (unverified) parts.push('AI 검증 호출이 실패해 ' + unverified + '문항은 "검증 보류"로 표시했습니다.')
