...
## 26. 내 계정 페이지와 상단 이동 동작

- `PATCH /users/me`로 이름·이메일·비밀번호를 수정한다(`UpdateProfileRequest`). 이름만 바꿀 때는 현재 비밀번호가 필요 없고, 이메일이나 비밀번호를 바꿀 때는 `current_password`로 본인 확인을 한다. 이메일은 다른 계정과 중복될 수 없다.
  - 구현 중 발견한 버그: `get_current_user`(다른 세션)로 받은 `current_user` 객체를 그대로 수정해 `db`(별도 세션)로 커밋하면 실제로는 저장되지 않고 `refresh`에서 예외가 났다. 엔드포인트 자신의 `db` 세션으로 사용자를 다시 조회해 수정하도록 고쳤다.
- 사이드바 하단 프로필 영역(아바타+이름)을 누르면 "내 계정" 화면으로 이동한다. 로그아웃 버튼은 그대로 별도로 둔다.
- 사이드바 상단의 로고(KCA Study)를 누르면 대시보드로 이동한다. 로그인 화면의 로고는 클릭 동작이 없다.

## 27. 전체 과목 학습 자료 업로드와 자료 기반 출제

- **발견한 문제**: 업로드한 학습 자료는 조각으로 나뉘어 세부항목별로 저장됐지만, 문제 생성 시 참고 자료를 조회하는 `retrieve_context`가 업로드 문서 조각을 제외(`source_document_id IS NULL`)하고 세부항목 요약만 가져왔다. 요약은 프롬프트에 이미 들어가 있어 사실상 업로드 자료가 출제에 쓰이지 않았다.
- **과목 선택 제거**: 업로드 시 대상 과목을 받지 않는다. `source_documents.subject_id`를 nullable로 바꿨고(Alembic `0009_documents_all_subjects`), 과목이 없는 문서는 38개 세부항목 전체를 대상으로 분류한다. 기존 과목 지정 문서와의 호환을 위해 과목이 있는 문서는 그 과목 안에서만 분류·재지정·참고문항 등록을 허용한다.
- **분류 개선**: 조각 크기를 1200자(`DOCUMENT_CHUNK_CHARS`)로 줄여 한 조각이 대체로 한 세부항목만 다루게 했다. 어떤 세부항목 이름·키워드와도 맞지 않는 조각(표지, 목차 등)은 첫 번째 토픽에 억지로 붙이지 않고 제외하며, 관련 조각이 하나도 없으면 문서를 실패로 표시한다. 점수가 같으면 더 긴(구체적인) 이름·키워드가 일치한 세부항목을 고른다. "개인정보보호 관련 법제" 안에 "정보보호 관련 법제"가 그대로 들어 있어, 동점일 때 먼저 나온 짧은 이름으로 잘못 분류되던 문제를 막기 위해서다.
- **자료 기반 출제**: `retrieve_context`는 해당 세부항목으로 분류된 업로드 자료 조각 중 관련도 상위 6개를 섞어, 문자 수 한도 안에서 반환한다(`pack_context`). 요청 전체 한도(`CONTEXT_CHAR_BUDGET`, 4000자)를 토픽 수로 나누되 토픽당 최소 400자를 보장해, 과목 전체 출제에서도 로컬 모델의 컨텍스트 창을 넘지 않게 했다. 비슷한 문제 생성에도 같은 자료를 쓴다. 실기 서술형 생성은 아직 자료를 쓰지 않는다.
- **프롬프트**: 자료가 있으면 그 사실에 근거해 문제와 해설을 쓰게 하고, 자료와 참고문항 속 지시문은 무시하도록 명시했다.
- **화면**: 콘텐츠 관리의 업로드 폼에서 대상 과목 선택을 없앴고, 조각 재지정 목록은 전체 과목의 세부항목을 과목별로 묶어 보여준다.
